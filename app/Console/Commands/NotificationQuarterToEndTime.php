...
class NotificationQuarterToEndTime extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'quarter:end';
    ...
    public function handle()
    {
        ...
        foreach ($tasks as $task) {
            ...
            if (!empty($startAt) && !empty($endTime)) {
                ...
                $halfway = $theStartTime->copy()->addSeconds($theEndTime->diffInSeconds($theStartTime) * 0.75);
                ...
                if ($now->between($windowStart, $windowEnd)) {
                    ...
                    if (isset($task->parent->parent->checklist->presetemplates) && is_iterable($task->parent->parent->checklist->presetemplates)) {
                        foreach ($task->parent->parent->checklist->presetemplates()->with('ntemp')->where('type', 3)->get() as $notification) {
