...
class ExportMonthlyReport implements ShouldQueue
{
    ...
    public function __construct($request, $eloquent)
    {
    ...
    public function handle()
    {
        ...
        $eloquent = MonthlyReportExport::find($this->eloquent);

        $eloquent->update([
            'status' => 1
        ]);

        $checklistsToInclude = DynamicForm::where('type', 0)->when($request['sop'] != 'all', function ($innerBuilder) use ($request) {
        ...
        foreach ($checklistsToInclude as $checklistsToIncludeRow) {
            ...
            ->scheduling()
            ->whereHas('parent.parent.checklist', function ($innerBuilder) use ($checklistsToIncludeRow, $request)  {
                return $innerBuilder->where('id', $checklistsToIncludeRow->id);
            })
            ->when($request['dom'] != 'all', function ($builder) use ($request) {
                $builder->whereHas('parent', function ($innerBuilder) use ($request) {
                    return $innerBuilder->where('user_id', $request['dom']);
                });
            })
            ->when($request['state'] != 'all' && !empty($request['state']), function ($builder) use ($request) {
                $builder->whereHas('parent.actstore.thecity', function ($innerBuilder) use ($request) {
                    return $innerBuilder->where('city_state', $request['state']);
                });
            })
            ->when($request['city'] != 'all' && !empty($request['city']), function ($builder) use ($request) {
                $builder->whereHas('parent.actstore', function ($innerBuilder) use ($request) {
