...
class ApiController extends \App\Http\Controllers\Controller
{
    ...
    public function tasks(Request $request)
    {
        ...
        ->when($request->task_type == 1 && in_array(request('filter_status'), ['PENDING', 'IN_PROGRESS', 'PENDING_VERIFICATION', 'REASSIGNED', 'VERIFIED']), function ($builder) {
        ...
        ->when($request->task_type == 2 && in_array(request('filter_status'), ['PENDING_VERIFICATION', 'REASSIGNED', 'VERIFIED']), function ($builder) {
            if (request('filter_status') == 'PENDING_VERIFICATION') {
                $builder->where('status', 2)->whereHas('ckrs', function ($query) {
        ...
        $tasks = $tasks
            ...
            ->map(function ($el) {
                ...
                $currentUserRoleForThisTask = [];

                if (isset($el->parent->parent->user_id) && $el->parent->parent->user_id == auth()->id()) {
                    $currentUserRoleForThisTask[] = 'maker';
                } else if (isset($el->ckrs) && $el->ckrs->contains('user_id', auth()->id())) {
                    $currentUserRoleForThisTask[] = 'checker';
                }
                ...
                if ($el->status == 0) {
                    ...
                    if (!(empty(request('task_type')) && empty(request('filter_status')))) {
                ...
                if (isset($versionedForm)) {
                    ...
                    if (!empty($el->data)) {
                        if ($reassigned) {
                            $latestDataofChecker = TaskChecker::select('data')->where('task_id', $el->id)->whereIn('status', [0, 1])->orderBy('level', 'ASC')->value('data');
                            $latestDataofChecker = is_array($latestDataofChecker) ? $latestDataofChecker : [];
                            $nonApprovedClasses = Helper::getNonApprovedFieldClassNames($latestDataofChecker);

                            foreach ($theFulfilledJson as $pageIndex => $pageData) {
                                if (is_array($pageData)) {
                                    $theFulfilledJson[$pageIndex] = array_values(array_filter($pageData, function ($item) use ($nonApprovedClasses) {
                                        return property_exists($item, 'className') && in_array($item->className, $nonApprovedClasses);
                                    }));
                                }
                            }

                            foreach ($latestDataofChecker as $row) {
                ...
                if (isset($el->parent->parent->checklist->id) && $el->parent->parent->checklist->is_store_checklist) {
                ...
                return [
                    'current_user_role_in_this_task' => $currentUserRoleForThisTask,

                    'scheduling_title' => $el->parent->parent->title ?? 'N/A',
                    'checklist_task_id' => $el->id,
                    'checklist_id' => $el->parent->parent->checklist_id ?? null,
                    ...
                    'is_checker' => isset($el->ckrs) && $el->ckrs->contains('user_id', auth()->id()),
    ...
    public function tickets(Request $request)
    {
        ...
        $query = NewTicket::query()
            ->with(['department', 'particular', 'issue', 'creator', 'store.store', 'store.docs', 'owners.user', 'histories.author'])
            ...
            ->when(auth()->check(), function ($inBldr) {
                ...
                if ($currentUserRole == 1) {
                    ...
                    $inBldr->where(function ($innerBuilder) {
                        $innerBuilder->whereHas('ckrs', function ($query) {
    ...
    public function checkerSubmission(Request $request)
    {
        ...
        if (is_numeric($currentUserCheckerLevel)) {
            ...
            if (($currentUserCheckerLevel - 1) >= 1) {
                ...
                \App\Jobs\ReassignmentNotification::dispatch($task, $cUser);

                if (TaskChecker::where('task_id', $request->task_id)->whereIn('status', [0, 1])->doesntExist()) {
                    $task->updated(['status' => 3]);
                }
    ...
    public function scanAssetLocation(Request $request) {
        $validator = Validator::make($request->all(), [
            'code' => 'required|exists:stores,ucode'
        ]);
        ...
        $asset = Store::where('ucode', $request->code)->withoutGlobalScope('os')->with(['store', 'subassets', 'docs.document', 'dom', 'storetype', 'modeltype', 'storecategory', 'assetStatus'])->first();

        if ($asset) {
            return response()->json(['success' => $asset]);
        }

        return response()->json(['error' => 'Give asset/location not found']);
    }

    public function assetCategories(Request $request) {
        $type = $request->get('type', 1);
        $categories = StoreCategory::select('id', 'name')->withoutGlobalScope('os')->where('type', $type)->get();
        return response()->json(['success' => $categories]);
    }

    public function assetMakes(Request $request) {
        $type = $request->get('type', 1);
        $makes = ModelType::select('id', 'name')->withoutGlobalScope('os')->where('type', $type)->get();
        return response()->json(['success' => $makes]);
    }

    public function assetModels(Request $request) {
        $type = $request->get('type', 1);
        $models = StoreType::select('id', 'name')->withoutGlobalScope('os')->where('type', $type)->get();
        return response()->json(['success' => $models]);
    }

    public function settings(Request $request) {
        $settings = Setting::first();
        return response()->json(['success' => $settings]);
    }
}