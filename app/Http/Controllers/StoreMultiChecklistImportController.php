            ...
            try {
                ...
                foreach ($data as $key => $row) {
                    ...
                    foreach ($allOfTheTemplates as $template) {
                        $makerBranch = $makerBranchType = null;
                        ...
                        $hRequiredRaw = is_numeric($row[5])
                            ? Date::excelToDateTimeObject($row[5])->format('H:i:s')
                            : '08:00';

                        $graceRaw = is_numeric($row[6])
                            ? Date::excelToDateTimeObject($row[6])->format('H:i:s')
                            : '08:00';

                        $weekDayTime = is_numeric($row[12])
                            ? Date::excelToDateTimeObject($row[12])->format('H:i:s')
                            : '00:00';
                        ...
                        foreach ($iterateNTimes as $iteratingStore) {
                            ...
                            $theFrequency = isset($frequencySlug[$row[9]]) ? $frequencySlug[$row[9]] : 12;
                            $theInterval = isset($row[10]) && is_numeric($row[10]) && $row[10] > 0 ? $row[10] : 0;
                            $perP = isset($row[13]) && strtolower($row[13]) == 'yes' ? 1 : 0;
                            ...
                            if ($theFrequency == 12) {
                                $checklistScheduling = ChecklistScheduling::create([
                                    ...
                                    'allow_rescheduling' => isset($row[7]) && strtolower($row[7]) == 'yes' ? 1 : 0,
                                    'is_import' => 1,
