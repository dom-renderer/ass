...
class ChecklistController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'checkers.*' => 'nullable|exists:users,id|distinct'
        ], [
            'checkers.*.distinct' => 'A user can only be assigned to one checker level.',
            'checkers.*.exists' => 'Selected user is invalid.'
        ]);
        ...
        try {
            ...
            $form->ptp = is_numeric($request->ptp) && $request->ptp >= 0 && $request->ptp <= 100 && $isPointChecklist ? $request->ptp : 0;
            $form->is_geofencing_enabled = $request->is_geofencing_enabled;
            $form->is_store_checklist = boolval($request->is_store_checklist);
            $form->needs_pass = boolval($request->needs_pass);
            ...
            if ($request->has('checkers') && is_array($request->checkers)) {
                ...
                foreach ($request->checkers as $userId) {
                    if (!empty($userId)) {
                        $checkersData[] = new \App\Models\ChecklistChecker([
                            'user_id' => $userId,
                            'level' => $level++
                        ]);
                    }
                }
                $form->checkers()->saveMany($checkersData);
            }
    ...
    public function select2List(Request $request)
    {
        ...
        if ($request->has('is_store_checklist') && $request->is_store_checklist == 1) {
            $query->where('is_store_checklist', 1);
        }

        if ($request->has('needs_pass') && $request->needs_pass == 1) {
            $query->where('needs_pass', 1);
        }
