...
class AssetController extends Controller
{
    public function index() {
        $page_title = "Stores";
        $stores = Store::withoutGlobalScope('os')->ass()
        ...
        ->when(!empty(request('filter_ucode')), function ($builder) {
            $builder->where('code', request('filter_ucode'));
        })
    ...
    public function store(Request $request) {
        ...
        $store = Store::create([
            ...
            'documents' => $this->uploadDocuments($request, 'documents'),
    ...
    public function update(Request $request, $stores) {
        ...
        Store::withoutGlobalScope('os')->ass()->where('id', $stores)->update([
            'store_type' => $request->store_type,
            'model_type' => $request->model_type,
            ...
            'documents' => $this->handleDocumentsUpdate($request, $stores),
            'asset_status_id' => $request->asset_status_id
        ]);
    ...
    private function uploadDocuments($request, $key) {
        $files = [];
    ...
    private function handleDocumentsUpdate($request, $id) {
        ...
        $newDocuments = $this->uploadDocuments($request, 'documents');
        if ($newDocuments) {
            $currentDocuments = array_merge($currentDocuments, $newDocuments);
        }

        return !empty($currentDocuments) ? array_values($currentDocuments) : null;
    ...
    public function select2List(Request $request) {
        ...
        if (!empty($queryString)) {
            $query->where(function ($builderQuery) use ($queryString) {
                $builderQuery->where('name', 'LIKE', "%{$queryString}%")->orWhere('code', 'LIKE', "%{$queryString}%");
            });
        }
    ...
    {
        $stores = Store::with( [ 'thecity', 'storetype' => function ($query) {
            $query->withoutGlobalScope('os')->ass();
        }, 'modeltype' => function ($query) {
            $query->withoutGlobalScope('os')->ass();
        }, 'dom', 'storecategory' => function ($query) {
            $query->withoutGlobalScope('os')->ass();
        } ] )
