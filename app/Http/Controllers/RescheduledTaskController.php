    ...
    public function index(Request $request) {
        if ($request->ajax()) {
            ...
            ->addColumn('action', function ($row) {
                $action = '';
                $checkerId = optional(optional(optional($row->task)->parent)->parent)->checker_user_id;
                $isAdmin = in_array(Helper::$roles['admin'], auth()->user()->roles()->pluck('id')->toArray());

                 if ($row->status === 0) {
                    // Use optional() to safely traverse the relationship chain in PHP 7.x
                    $checkerId = optional(optional(optional($row->task)->parent)->parent)->checker_user_id;

                    $isAdmin = in_array(Helper::$roles['admin'], auth()->user()->roles()->pluck('id')->toArray());

                    if ($checkerId == auth()->user()->id || $isAdmin) {
                        $action .= '
                    ...
                    $action .= ($row->status == 1) ? "Approved" : "Rejected";
                }
