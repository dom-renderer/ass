    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function ($row) use ($thisUserRoles, $currentUser) {
                    ...
                    if (auth()->user()->can('scheduled-tasks.show')) {
                        ...
                        if (is_iterable($row->ckrs) && !empty($row->ckrs)) {
                            foreach ($row->ckrs as $cList) {
                                if ($cList->status == 1 || $cList->status == 2) {
                                    $dropdownItems .= '<li><a class="dropdown-item" style="background:#c8ceff;" href="'.route('checker-submisison', ['task' => encrypt($row->id),  'user' => encrypt($cList->user_id)]).'">Checker ' . $cList->level . ' Submision</a></li>';
                                }
                            }
                        }
                    }
                ...
                if ($row->status == 0) {
                    $dropdownItems .= '<li><a class="dropdown-item" href="'.route("task-checkers", encrypt($row->id)).'">Manage Checkers</a></li>';
                }
