...
class NewTicket extends Model
{
    ...
    {
        return $this->belongsTo(Store::class)->withoutGlobalScope('os');
    }
    ...
    public function getAttachmentLinksAttribute() {
        ...
        $attachment_arr = !empty($this->attachments) ? $this->attachments : array();
        if ( !empty($attachment_arr) ) {
            foreach ( $attachment_arr as $attachment ) {
                if ( file_exists( public_path( "storage/new-ticket-attachments/{$attachment}" ) ) ) {
                    $attachment_url_arr[] = asset( "storage/new-ticket-attachments/{$attachment}" );
                }
            }
        }
