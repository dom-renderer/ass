    ...
    <div class="bg-light p-4 rounded">
        ...
        <div class="container mt-4">
            ...
            <form method="POST" action="{{ route( 'document-upload.update', encrypt( $documentupload->id ) ) }}" id="documentUploadForm" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label class="form-label" for="zp_location">Location/Asset<span class="text-danger"> *</span></label>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="proposal_number" id="proposal_number" class="form-control" placeholder="Enter Proposal Number">{{ old('proposal_number', $documentupload->proposal_number ?? '') }}</textarea>

                    @if ( $errors->has( 'proposal_number' ) )
                        <span class="text-danger text-left">{{ $errors->first( 'proposal_number' ) }}</span>
                    @endif
                </div>
                ...
                <div class="mb-3">
                    <label class="form-label" for="zp_document_owners">Document Owners</label>
                    <select name="zp_document_owners[]" id="zp_document_owners" multiple>
                        @if ( !empty($ownerUsers) )
                            @foreach ( $ownerUsers as $user_row )
                                <option value="{{ $user_row->id }}" selected>{{ (!empty($user_row->employee_id) ? "{$user_row->employee_id} - " : '') . "{$user_row->name} {$user_row->middle_name} {$user_row->last_name}" }}</option>
                            @endforeach
                        @endif
                    </select>
...
@push('js')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#zp_document_owners').select2({
                placeholder: 'Select Document Owners',
                allowClear: true,
                width: '100%',
                theme: 'classic',
                ajax: {
