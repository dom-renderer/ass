...
@section('content')
    <div class="px-6 pt-6 pb-10">
        <form method="POST" action="{{ route('asset-statuses.update', encrypt($assetStatus->id)) }}"
            class="max-w-3xl mx-auto">
            @method('patch') @csrf
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800">Edit Asset Status</h2>
                    <p class="text-sm text-gray-400 mt-0.5">Modify this status label.</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit"
                        class="inline-flex items-center gap-2 px-5 py-2 bg-primary text-white text-sm font-medium rounded-lg transition-colors shadow-sm cursor-pointer"><i
            ...
            <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $assetStatus->title) }}"
                            class="form-input" placeholder="Enter title" required>
                    ...
                    <div>
                        ...
                        <input type="color" name="color" id="color" value="{{ old('color', $assetStatus->color) }}"
                            class="form-input" style="height: 42px; padding: 4px;" required>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-6 mt-1">
                            <label class="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                                <input type="radio" name="type" value="1" {{ old('type', $assetStatus->type) == '1' ? 'checked' : '' }} class="accent-blue-600"> Deployable
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                                <input type="radio" name="type" value="2" {{ old('type', $assetStatus->type) == '2' ? 'checked' : '' }} class="accent-blue-600"> Undeployable
                            </label>
                        </div>
