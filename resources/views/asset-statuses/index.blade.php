...
@section('content')
    <div class="px-6 pt-6 pb-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $page_title ?? 'Asset Statuses' }}</h2>
                <p class="text-sm text-gray-400 mt-0.5">{{ $page_description ?? 'Manage asset lifecycle statuses.' }}</p>
            </div>
    ...
    <script>
        $(document).ready(function ($) {
            ...
            let usersTable = new DataTable('#asset-status-table', {
                ...
                columns: [
                    { data: 'title' }, { data: 'color' }, { data: 'type' }, { data: 'action', className: 'text-center' }
                ]
            });
        });
    </script>
