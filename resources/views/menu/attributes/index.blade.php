...
<div class="bg-light p-4 rounded">
    ...
    <div class="lead d-flex flex-wrap align-items-center gap-2">
        @can('menu_attributes.create')
            <a href="{{ route('menu.attributes.create') }}" class="btn btn-primary btn-sm ms-auto">Add Attribute</a>
        @endcan
    </div>
    @include('layouts.partials.messages')
    <table class="table table-striped w-100" id="attr-table">
        <thead>
            <tr>
                <th>Name</th>
                <th>Status</th>
...
$(function () {
    ...
    let table = new DataTable('#attr-table', {
        dom: '<"d-flex justify-content-between mb-2"f>rtpi',
        processing: true,
        serverSide: true,
        ajax: {
            url: '{{ route('menu.attributes.index') }}'
        },
        columns: [
            { data: 'name' },
    ...
    $(document).on('click', '.btn-restore-attr', function () {
        const id = $(this).data('id');
        $.post('{{ url('menu/attributes') }}/' + id + '/restore', { _token: csrf }, function () {
