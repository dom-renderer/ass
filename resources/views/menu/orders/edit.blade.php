...
<div data-ofs>
  ...
  <div class="ofs-header">
    <div>
      <h1>{{ $page_title }}</h1>
      <p class="ofs-breadcrumb">Orders &rsaquo; {{ isset($order) ? 'Edit #'.$order->id : 'New Order' }}</p>
    </div>
  </div>

  <form method="POST" action="{{ route('menu.orders.update', $order->id) }}" id="orderForm" autocomplete="off">
    @csrf
    @method('PUT')
    ...
    <div class="ofs-card">
      ...
      <div class="ofs-card-body">
        ...
        <button type="button" id="addItemCard" class="ofs-add-item-btn">
          <span style="font-size:17px; line-height:1;">+</span> Add Item
        </button>
      </div>
    </div>
    ...
    <div class="ofs-action-bar">
      <button type="submit" class="ofs-btn btn-primary">
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2H5a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V5l-3-3z"/><polyline points="13 5 10 5 10 2"/><line x1="7" y1="9" x2="9" y2="9"/><line x1="8" y1="8" x2="8" y2="10"/></svg>
        Save Order
      </button>
      <a href="{{ route('menu.orders.index') }}" class="ofs-btn ofs-btn-ghost">
        &larr; Back to Orders
      </a>
    </div>
  ...
  <div class="ofs-item-card item-card" data-idx="${idx}">
    <div class="ofs-item-grid">
      ...
      <div class="ofs-field" style="display:flex;align-items:flex-end;">
        <button type="button" class="ofs-btn-danger-icon remove-item" title="Remove item">
