...
<div class="bg-light p-4 rounded shadow-sm border-0">
    ...
    <form method="POST" action="{{ route('menu.products.update', $product->id) }}" enctype="multipart/form-data" id="productForm" class="mt-3">
        @csrf
        @method('PUT')
        <div class="row g-3">
            <div class="col-lg-6">
                <div class="card mb-3 shadow-sm border-0">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $product->name) }}" maxlength="191" required>
                            @error('name')<span class="text-danger small d-block mt-1">{{ $message }}</span>@enderror
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Slug</label>
                            <input type="text" name="slug" class="form-control" value="{{ old('slug', $product->slug) }}" maxlength="191">
                        ...
                        <div class="mb-3">
                            ...
                            @if($product->image)
                                <div class="mb-2"><img src="{{ \Illuminate\Support\Facades\Storage::url('menu/products/'.$product->image) }}" class="img-thumbnail" style="max-height:120px" alt=""></div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="ordering" class="form-control" value="{{ old('ordering', $product->ordering) }}" min="0" required>
...
<script type="application/json" id="attributes-json-edit">@json($attributes)</script>
<script type="application/json" id="addons-json-edit">@json($addons)</script>
<script type="application/json" id="existing-attrs-json-edit">@json($existingProductAttributes)</script>
<script type="application/json" id="existing-addons-json-edit">@json($existingAddonPrices ?? [])</script>
@endsection

@push('js')
<script src="{{ asset('assets/js/select2.min.js') }}"></script>
<script>
const attributesData = JSON.parse(document.getElementById('attributes-json-edit').textContent || '[]');
const addonsData = JSON.parse(document.getElementById('addons-json-edit').textContent || '[]');
const existingAttrs = JSON.parse(document.getElementById('existing-attrs-json-edit').textContent || '[]');
const existingAddonMap = JSON.parse(document.getElementById('existing-addons-json-edit').textContent || '{}');
const attributeState = {};
const addonState = {};

function seedStates() {
    (existingAttrs || []).forEach(r => {
        const aid = String(r.attribute_id), vid = String(r.attribute_value_id);
        if (!attributeState[aid]) attributeState[aid] = { values: {} };
        attributeState[aid].values[vid] = {
            price_override: r.price_override ?? '',
            is_available: r.is_available !== false,
            is_default: !!r.is_default
        };
    });
    Object.keys(existingAddonMap || {}).forEach(id => {
        const row = existingAddonMap[id] || {};
        addonState[String(id)] = {
...
$(function () {
    ...
    $('#attribute_selector').val(Object.keys(attributeState)).trigger('change.select2');
    $('#addon_selector').val(Object.keys(addonState)).trigger('change.select2');
    renderAttributes();
    renderAddons();
