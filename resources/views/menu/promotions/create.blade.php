...
<style>
    ...
    .promo-help { font-size: .82rem; color: #6b7280; }
</style>
@endpush
...
<div class="bg-light p-4 rounded shadow-sm border-0">
    <div class="promo-header rounded p-3 mb-3">
        ...
        <div class="small">Build high-converting offers like Swiggy/Zomato campaigns</div>
    </div>
    @include('layouts.partials.messages')
    <form method="POST" action="{{ route('menu.promotions.store') }}" id="promotionForm">
        @csrf
        ...
        <div class="card promo-card mb-3">
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-3"><label class="form-label">Offer Type *</label>
                        <select name="type" id="type" class="form-control" required>
                            @foreach(\App\Models\Promotion::TYPES as $type)
                                <option value="{{ $type }}" {{ old('type', $promotion->type ?? '') === $type ? 'selected' : '' }}>{{ strtoupper(str_replace('_',' ', $type)) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="alert alert-primary py-2 mb-0">
                            <strong>Offer Preview:</strong> <span id="offer_preview_chip">Set values to see offer summary.</span>
                        </div>
                    </div>
                    <div class="col-md-12">
        ...
        <div class="card promo-card mb-3" id="discount_block">
            <div class="card-body">
        ...
        <div class="card promo-card mb-3" id="bxgy_block">
            <div class="card-body">
        ...
        <div class="card promo-card mb-3" id="scope_block">
            <div class="card-body">
        ...
        <div class="card promo-card mb-3" id="rule_builder_block">
            <div class="card-body">
                ...
                <input type="hidden" name="rule_builder" id="rule_builder_json" value="{{ old('rule_builder', '[]') }}">
            </div>
        </div>
        ...
        <div class="mt-3 d-flex gap-2">
            <button class="btn btn-primary"><i class="fa-solid fa-floppy-disk me-1"></i>Save Promotion</button>
            <a href="{{ route('menu.promotions.index') }}" class="btn btn-secondary">Back</a>
        </div>
    </form>
</div>
@endsection
...
$(function(){
    ...
    function updatePreview() {
        ...
        if (type === 'cart_percent' && d !== null) {
            text = 'Flat ' + d + '% off' + (maxD !== null ? ' up to ' + maxD : '') + (minC !== null ? ' on orders above ' + minC : '') + '.';
        } else if (type === 'cart_flat' && d !== null) {
            text = 'Flat ' + d + ' off' + (minC !== null ? ' on orders above ' + minC : '') + '.';
        } else if ((type === 'category_percent' || type === 'product_percent') && d !== null) {
            text = d + '% off on selected ' + (type.indexOf('category') === 0 ? 'categories' : 'products') + (maxD !== null ? ' up to ' + maxD : '') + '.';
        } else if ((type === 'category_flat' || type === 'product_flat') && d !== null) {
            text = 'Flat ' + d + ' off on selected ' + (type.indexOf('category') === 0 ? 'categories.' : 'products.');
        } else if (type === 'bxgy') {
            text = 'Buy ' + (buyQ || 'X') + ' and get ' + (getQ || 'Y') + ' offer.';
        } else if (type === 'free_delivery') {
            text = 'Free delivery offer.';
        } else if (type === 'first_order' && d !== null) {
            text = 'First order: flat ' + d + (maxD !== null ? ' up to ' + maxD : '') + '.';
        }
        $('#offer_preview_chip').text(text);
    }

    function hint(msg, isErr) {
        $('#inline_validation_hint').text(msg).toggleClass('text-danger', !!isErr).toggleClass('text-success', !isErr);
    }
    ...
    function validateHuman() {
        ...
        hint('Looks good. Promotion is valid for save.', false);
        return true;
    }
    ...
    function syncRuleJson() {
        ...
        $('#rule_rows .rule-row').each(function(){
            rows.push({
                condition: $(this).find('.rule-condition').val(),
                operator: $(this).find('.rule-operator').val(),
                value: $(this).find('.rule-value').val(),
                logic: $(this).find('.rule-logic').val()
            });
        });
    ...
    function applyPreset(key) {
        if (!key) return;
        if (key === 'new_user_cart20') {
            $('#type').val('cart_percent').trigger('change');
            $('[name="discount_value"]').val('20');
            $('[name="max_discount_amount"]').val('100');
            $('[name="min_cart_amount"]').val('299');
            $('#rule_rows').empty();
            addRuleRow({condition:'user_type',operator:'=',value:'new',logic:'AND'});
        } else if (key === 'weekend_flat75') {
            $('#type').val('cart_flat').trigger('change');
            $('[name="discount_value"]').val('75');
            $('[name="min_cart_amount"]').val('499');
            $('#rule_rows').empty();
            addRuleRow({condition:'day_of_week',operator:'in',value:'sat,sun',logic:'AND'});
        } else if (key === 'lunch_rush_15') {
            $('#type').val('category_percent').trigger('change');
            $('[name="discount_value"]').val('15');
            $('[name="max_discount_amount"]').val('120');
            $('#rule_rows').empty();
            addRuleRow({condition:'day_of_week',operator:'in',value:'mon,tue,wed,thu,fri',logic:'AND'});
        } else if (key === 'bxgy_classic') {
            $('#type').val('bxgy').trigger('change');
            $('[name="buy_quantity"]').val('2');
            $('[name="get_quantity"]').val('1');
            $('#rule_rows').empty();
            addRuleRow();
        }
        updatePreview();
    ...
    try {
        ...
        if (Array.isArray(existingRules) && existingRules.length) {
            existingRules.forEach(addRuleRow);
        } else {
            addRuleRow();
        }
    } catch (e) {
    ...
    $('#promotionForm').on('submit', function(e){
        ...
        if (!validateHuman()) {
            e.preventDefault();
            Swal.fire('Validation', 'Please resolve highlighted rule hints before saving.', 'warning');
        }
    });
