...
@section('content')
    <form method="post" action="{{ route('users.update', $user->id) }}" enctype="multipart/form-data"
        class="edituser px-6 pt-6 pb-10">
        @method('patch')
        @csrf
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Edit User Profile</h2>
                <p class="text-sm text-gray-400 mt-0.5">Modify the information and system access settings for this user.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                            <div>
                                ...
                                <input value="{{ old('name', $user->name) }}" type="text" class="form-input" name="name"
                            ...
                            <div>
                                ...
                                <input value="{{ old('middle_name', $user->middle_name) }}" type="text" class="form-input"
                            ...
                            <div>
                                ...
                                <input value="{{ old('last_name', $user->last_name) }}" type="text" class="form-input"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <input value="{{ old('employee_id', $user->employee_id) }}" type="text" class="form-input"
                                    name="employee_id" placeholder="Ex: EMP-001">
                            ...
                            <div>
                                ...
                                <input value="{{ old('profile') }}" type="file" class="form-input file-input" name="profile"
                                    accept="image/*">
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <input value="{{ old('email', $user->email) }}" type="email" class="form-input" name="email"
                            ...
                            <div>
                                ...
                                <input value="{{ old('username', $user->username) }}" type="text" class="form-input"
                                    name="username" placeholder="Unique username" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input value="{{ old('phone_number', $user->phone_number) }}" type="text" class="form-input"
                                    name="phone_number" placeholder="(555) 555-5555" required>
                            ...
                            <div>
                                ...
                                <input value="{{ old('password') }}" type="password" class="form-input" name="password"
                ...
                <div id="particular-view"
                    class="@if(!in_array(1, $user->tRoles()->pluck('ticket_role_id')->toArray())) hidden @endif">
                    <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden mt-2">
                                                        ...
                                                        <div
                                                            ...
                                                            <label class="switch-toggle" style="width: 32px; height: 18px;">
                                                                <input type="checkbox" class="chckbx{{ $department->id }}"
                                                                    ...
                                                                    id="issuesChk{{ $issue->id }}" @if(in_array($issue->id, $allIssues))
                                                                    checked @endif>
            ...
            <div class="space-y-6">
                    ...
                    <div class="p-5 space-y-5">
                        <div>
                            ...
                            <select class="form-select" name="status" id="status">
                                <option value="1" <?php echo $status = $user->status == 1 ? "selected='selected'" : ''; ?>>
                                    Enabled</option>
                                <option value="0" <?php echo $status = $user->status == 0 ? "selected='selected'" : ''; ?>>
                                    Disabled</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select class="form-select" id="role" name="role" required>
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" @if(isset($user->roles[0]->id) && $role->id == $user->roles[0]->id) selected @endif>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- Dynamic JS containers pre-filled --}}
                        <div id="dynamic-dom-options">
                            @if(!empty($selectedDoms))
                                <div class="mt-4">
                                    <label class="form-label"> Assign DoM </label>
                                    <select name="doms[]" id="doms" multiple>
                                        @forelse ($selectedDoms as $value)
                                            @if(isset($value['dom']['id']))
                                                <option value="{{ $value['dom']['id'] }}" selected> {{ $value['dom']['employee_id'] }} -
                                                    {{ $value['dom']['name'] }} {{ $value['dom']['middle_name'] }}
                                                    {{ $value['dom']['last_name'] }} </option>
                                            @endif
                                        @empty
                                        @endforelse
                                    </select>
                            ...
                            @elseif(isset($user->roles[0]->id) && in_array($user->roles[0]->id, [6, 10]))
                                <div class="mt-4">
                                    <label class="form-label"> Assign DoM </label>
                                    <select name="doms[]" id="doms" multiple></select>
                                </div>
                            @endif
                        </div>

                        <div id="dynamic-role-options">
                            @if(!empty($store))
                                <div class="mt-4">
                                    <label class="form-label"> Organizations / Departments </label>
                ...
                <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                    ...
                    <div class="p-5 space-y-5">
                        <div class="ticket-settings">
                            ...
                            <select id="ticket_roles" name="ticket_roles[]" multiple="multiple">
                                @foreach ($tRoles as $tRole)
                                    <option value="{{ $tRole->id }}" @if(in_array($tRole->id, $user->tRoles()->pluck('ticket_role_id')->toArray())) selected @endif>
                                        {{ $tRole->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div id="store-view"
                            class="@if(!in_array(2, $user->tRoles()->pluck('ticket_role_id')->toArray())) hidden @endif mt-2">
                            ...
                            <select id="ticket-dom-stores" name="dom_stores[]" multiple="multiple">
                                @foreach (\App\Models\Store::where(function ($innerBuilder) {
                                            $innerBuilder->whereNull('dom_id')->orWhere('dom_id', '');
...
@push('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            // Pre-initialize doms if present
            if ($('#doms').length > 0) {
                initDomsSelect();
            }
            ...
            $('#role').select2({
                ...
                if (!isNaN(selectedRole)) {
                    if (['2', '3', '4'].includes(selectedRole)) {
                        ...
                        `);
                        initStoreDepartmentSelect('Select a Location');
                        ...
                        `);
                        initStoreDepartmentSelect('Select a Office');
                        ...
                        `);
                        initStoreDepartmentSelect('Select a Department');
                    } else {
                        $('#dynamic-role-options').html('');
                    }
                }
            ...
            function initStoreDepartmentSelect(placeholder) {
            ...
            function getAjaxUrl() {
                let roleId = $('#role').val() || "{{ isset($user->roles[0]->id) ? $user->roles[0]->id : '' }}";
                if (['2', '3', '4', '11'].includes(roleId)) return "{{ route('stores-list') }}";
                if (['5'].includes(roleId)) return "{{ route('corporate-offices-list') }}";
                if (['6', '7', '10'].includes(roleId)) return "{{ route('departments-list') }}";
                return "";
            }

            // Initialize the select2 if it exists on load
            if ($('#thisStoreDepartmentCOffice').length > 0) {
                initStoreDepartmentSelect('Select an option');
            }
        });
    </script>
@endpush