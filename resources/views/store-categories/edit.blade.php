...
<div class="px-6 pt-6 pb-10">
    <form method="POST" action="{{ route('store-categories.update', $id) }}" id="categoryForm" class="max-w-3xl mx-auto">
        @csrf @method('PUT')

        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Edit Location Category</h2>
                <p class="text-sm text-gray-400 mt-0.5">Modify this Location Category entry.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('store-categories.index') }}" class="px-4 py-2 border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition-colors shadow-sm text-decoration-none">
                    Cancel
                </a>
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-2">
                ...
                <h3 class="text-sm font-semibold text-gray-800 m-0">Location Category Details</h3>
            </div>
            <div class="p-5">
                <div>
                    <label for="name" class="form-label">Name <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="name" value="{{ old('name', $storecategory->name) }}" class="form-input" placeholder="Enter name" required>
                    @if ($errors->has('name'))
