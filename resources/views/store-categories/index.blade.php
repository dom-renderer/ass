...
@section('content')
    ...
    <div class="px-6 pt-6 pb-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $page_title ?? 'Location Categories' }}</h2>
                <p class="text-sm text-gray-400 mt-0.5">{{ $page_description ?? 'Manage Location Categories in the system.' }}</p>
            </div>
            <div class="flex items-center mt-4 md:mt-0">
                @if (auth()->user()->can('store-categories.create'))
                    <a href="{{ route('store-categories.create') }}"
                        ...
                        <i class="bi bi-plus-lg"></i> Add Location Category
                    </a>
                @endif
            </div>
        </div>
            ...
            <div class="tab-pane fade show active" id="users-tab-pane" role="tabpanel" aria-labelledby="users-tab" tabindex="0">
                <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-2 bg-gray-50/50">
                        ...
                        <h3 class="text-sm font-semibold text-gray-800 m-0">Location Categories Directory</h3>
                    </div>
...
<script>
    $(document).ready(function($){
        ...
        $(document).on('click', '.deleteGroup', function(e) {
            e.preventDefault();
            Swal.fire({
                title: 'Are you sure you want to delete this Location Category?',
