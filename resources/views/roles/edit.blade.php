...
@push('css')
    ...
    <style>
        ...
        .form-input[readonly] { background-color: #f9fafb; cursor: not-allowed; opacity: 0.8; }
...
<div class="px-6 pt-6 pb-10">
    <form method="POST" action="{{ route('roles.update', $role->id) }}" class="">
        @method('PATCH')
        @csrf
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Edit Role Settings</h2>
                <p class="text-sm text-gray-400 mt-0.5">Modify permissions and access layers for this role.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('roles.index') }}" class="px-4 py-2 border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition-colors shadow-sm text-decoration-none">
                    Cancel
                </a>
                <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden w-full lg:w-1/2">
                ...
                <div class="p-5">
                    ...
                    <input value="{{ $role->name }}" type="text" class="form-input" name="name" id="name" required @if(in_array($role->id, [1,2,3])) readonly @endif>
                </div>
            </div>
                ...
                <div class="p-6 bg-gray-50/30">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach($permissionGroups as $group => $groupPermissions)
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow flex flex-col h-full overflow-hidden">
                                ...
                                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <label class="text-xs font-medium text-gray-500 cursor-pointer mb-0" for="group-{{ $group }}">All</label>
                                        <label class="switch-toggle" style="width: 30px; height: 16px;">
                                            <input type="checkbox" class="group-check" data-group="{{ $group }}" id="group-{{ $group }}" {{ $groupChecked ? 'checked' : '' }}>
                                        ...
                                        <div class="flex items-center gap-3">
                                            <label class="switch-toggle" style="width: 30px; height: 16px;">
                                                <input type="checkbox"
                                                       ...
                                                       class="permission permission-{{ $group }}"
                                            ...
                                            <label class="text-sm font-medium text-gray-600 m-0 cursor-pointer flex-1" for="permission-{{ $permission->id }}">
                                                {{ $permission->title }}
                                            </label>
                                        </div>
                                    @endforeach
