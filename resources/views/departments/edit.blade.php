...
<div class="px-6 pt-6 pb-10">
    <form method="POST" action="{{ route('departments.update', $id) }}" class="max-w-3xl mx-auto">
        @csrf @method('PUT')

        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Edit Department</h2>
                <p class="text-sm text-gray-400 mt-0.5">Modify this Department entry.</p>
            </div>
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
            ...
            <div class="p-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $department->name) }}" class="form-input" placeholder="Enter name" required>
                    @if ($errors->has('name'))
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="description" id="description" class="form-textarea" placeholder="Enter a brief description">{{ old('description', $department->description ?? '') }}</textarea>
                    </div>
            </div>
        </div>
    </form>
