...
@section('content')
    <div class="px-6 pt-6 pb-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">{{ $page_title ?? 'Particulars' }}</h2>
                <p class="text-sm text-gray-400 mt-0.5">
                    {{ $page_description ?? 'Manage particulars linked to departments.' }}</p>
            </div>
            <div class="flex items-center mt-4 md:mt-0">
                @if (auth()->user()->can('particulars.create'))
                    <a href="{{ route('particulars.create') }}"
                        ...
                        <i class="bi bi-plus-lg"></i> Add Particular
                    </a>
                @endif
            </div>
        </div>
            ...
            <div class="tab-pane fade show active" role="tabpanel" tabindex="0">
                <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-2 bg-gray-50/50">
                        ...
                        <h3 class="text-sm font-semibold text-gray-800 m-0">Particulars Directory</h3>
                    </div>
                    <div class="p-0 table-responsive">
                        <table class="table table-striped w-100 m-0" id="particulars-table" cellspacing="0">
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Department</th>
        ...
        $(document).ready(function () {
            $(document).on('click', '.deleteGroup', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) { $(this).parents('form').submit(); return true; } else { return false; }
                })
            });
            ...
            let table = new DataTable('#particulars-table', {
                ...
                processing: false, ordering: false, serverSide: true,
                columns: [
                    { data: 'name' },
