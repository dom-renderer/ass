...
@section('content')
    <div class="px-6 pt-6 pb-10">
        <form method="POST" action="{{ route('particulars.update', $id) }}" id="particularForm" class="max-w-3xl mx-auto">
            @csrf @method('PUT')
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800">Edit Particular</h2>
                    <p class="text-sm text-gray-400 mt-0.5">Modify this particular entry.</p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $particular->name) }}"
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#department').select2({
                ...
                ajax: {
                    ...
                    data: function (params) {
                        return { searchQuery: params.term, page: params.page || 1, _token: "{{ csrf_token() }}" };
                    },
            ...
            $('#particularForm').validate({
                rules: { name: { required: true }, department: { required: true } }
            });
        });
    </script>
@endpush