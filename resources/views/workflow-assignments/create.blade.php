...
@section('content')
    <div class="p-4 rounded">
        <form action="{{ route('workflow-assignments.store') }}" method="post" id="templateForm" novalidate>
            @csrf
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-12">
                    <label class="form-label">Project Template <span class="text-danger"> * </span> </label>
                    <select name="new_workflow_template_id" id="workflow_template_id" class="select2" required>
                        <option value="">Select a Template</option>
                        @foreach($templates as $t)
                            <option value="{{ $t->id }}" {{ old('new_workflow_template_id', request('template_id')) == $t->id ? 'selected' : '' }}>{{ $t->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="start_from" id="start_from" class="form-control" value="{{ old('start_from', date('Y/m/d H:i')) }}" required readonly>
                </div>

                <div class="col-md-12">
                    <label class="form-label">Title <span class="text-danger"> * </span> </label>
                    <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $template->title ?? '') }}" required>
                </div>
                ...
                <div class="col-md-12 mt-2">
                    <input type="checkbox" style="height:20px;width:20px;" value="1" name="on_going_project" id="on_going_project">
                    <label class="form-label" for="on_going_project" style="position: relative;bottom: 5px;" > Is On-going Project </label>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            if (sectionsData && typeof sectionsData === 'object') {
                Object.values(sectionsData).forEach(section => {
                    if (section.steps && !Array.isArray(section.steps)) {
                        section.steps = Object.values(section.steps);
                    }
                });
            }
            ...
            $('#workflow_template_id').select2({
                ...
                let selectedTemplate = $('option:selected', this).val();

                if (currentUrl.indexOf('?') > -1) {
                    currentUrl += `&template_id=${selectedTemplate}`;
                } else {
                    currentUrl += `?template_id=${selectedTemplate}`;
                }
            ...
            function renderSectionSidebarItem(sectionId, section) {
                const sectionNumber = Object.keys(sections).indexOf(sectionId) + 1;
                const stepsCount = section.steps ? section.steps.length : 0;
                    ...
                    <div class="section-item" data-section-id="${sectionId}">
                        <div class="d-flex align-items-center p-3">
                            ...
                            <div class="flex-grow-1">
                                <div class="fw-semibold section-name">${section.name || 'New Department'}</div>
                                <div class="section-code text-muted">${section.code || 'NEW_DEPARTMENT'}</div>
                                <div class="text-muted small section-description">${section.description || 'No description provided'}</div>
                                <div class="d-flex justify-content-between align-items-center mt-1">
                                    <span class="section-steps-count">${stepsCount} tasks</span>
            ...
            // Render existing sections on load
            if (sections && typeof sections === 'object') {
                Object.values(sections).forEach(section => {
                    renderSectionSidebarItem(section.id, section);
                });
            ...
            function addSection() {
                ...
                renderSectionSidebarItem(sectionId, sectionData);
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-md-12">
                                                        ...
                                                        <div class="form-hint">
                                                            ...
                                                            Use department-oriented names that clearly indicate the department's purpose
                                                        </div>
                                                    </div>
                ...
                if (validateForm()) {
                    ...
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            on_going_project: $('#on_going_project').is(':checked'),
            ...
            function displayValidationErrors(errors) {
                ...
                if (stepErrors.length > 0) {
                    errorHtml += `
                        <div class="error-group mb-3">
                            <h6 class="d-flex align-items-center mb-2">
                                <i class="bi bi-list-check me-2"></i>Task Configuration
                            </h6>
                            <ul class="mb-0 ps-4">
            ...
            function makeFriendlyErrorMessage(key, message) {
                const friendlyNames = {
                    ...
                    'maker_escalation_email_notification': 'Maker Escalation Email Notification Template',
                    'maker_escalation_push_notification': 'Maker Escalation Push Notification Template',
                    'maker_completion_email_notification': 'Maker Completion Email Notification Template',
                    'maker_completion_push_notification': 'Maker Completion Push Notification Template',
                    'maker_dependency_email_notification': 'Maker Dependency Email Notification Template',
                    'maker_dependency_push_notification': 'Maker Dependency Push Notification Template',
                    'checker_id': 'Checker',
                    ...
                    'checker_escalation_email_notification': 'Checker Escalation Email Notification Template',
                    'checker_escalation_push_notification': 'Checker Escalation Push Notification Template',
                };
