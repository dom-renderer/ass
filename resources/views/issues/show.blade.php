    ...
    <div class="px-6 pt-6 pb-10">
        <div class="max-w-3xl mx-auto">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800">View Issue</h2>
                    <p class="text-sm text-gray-400 mt-0.5">Read-only view of this issue.</p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-2">
                    <i class="bi bi-exclamation-triangle text-gray-400 text-lg"></i>
                ...
                <div class="p-6 space-y-4">
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-5">
                        ...
                        <span class="block text-lg text-gray-800 font-medium">{{ $issue->name ?? '-' }}</span>
                    </div>
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-5">
                        <span
                            class="block text-xs font-semibold text-gray-400 tracking-wide uppercase mb-1">Department</span>
                        <span
                            class="block text-sm text-gray-700">{{ optional(\App\Models\Department::withTrashed()->find($issue->department_id))->name ?? '-' }}</span>
                    </div>
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-5">
                        <span
                            class="block text-xs font-semibold text-gray-400 tracking-wide uppercase mb-1">Particular</span>
                        <span
                            class="block text-sm text-gray-700">{{ optional(\App\Models\Particular::withTrashed()->find($issue->particular_id))->name ?? '-' }}</span>
                    </div>
                    <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-5">
                        <span class="block text-xs font-semibold text-gray-400 tracking-wide uppercase mb-1">Status</span>
                        <span class="block text-sm text-gray-700">{{ $issue->status ? 'Active' : 'InActive' }}</span>
                    </div>
                </div>
            </div>
        </div>
